...
namespace PocoRender.UI.Modules
{
    public class HomeModule
    {
        ...
        public void CreateUVPrintLayout(GameObject parent, PocoRenderStudioUIManager manager, System.Action<Color?> addCanvasCallback)
        {
            ...
            var pubBtn = UIFactory.CreateButton("Publish", subRight, Vector2.zero, new Vector2(80, 32), Color.black, Color.white);
            pubBtn.GetComponent<Button>().onClick.AddListener(() => OnPublishClick(layout.gameObject));
        ...
        private void OnPublishClick(GameObject root)
        {
            if (activeController == null || activeController.paper == null) return;

            var bridge = root.GetComponentInParent<Communication.QtBridgeController>();
            if (bridge == null) bridge = Object.FindObjectOfType<Communication.QtBridgeController>();
            if (bridge == null || !bridge.IsConnected)
            {
                Debug.LogWarning("[HomeModule] Cannot publish - bridge not connected");
                return;
            }
            ...
            pr.sizeDelta = new Vector2(1000, 650); // Reduced height to fit screen
            ...
            hr.sizeDelta = new Vector2(0, 40); // Reduced header height
            hr.anchoredPosition = new Vector2(0, 0);

            // Title
            GameObject titleObj = UIFactory.CreateText("Publish a 2D Design", header, 18, Color.black, new Vector2(0, 0), new Vector2(0, 0), TextAnchor.MiddleLeft, FontStyle.Normal);
            titleObj.GetComponent<RectTransform>().offsetMin = new Vector2(20, 0); // Padding left
            titleObj.AddComponent<LayoutElement>().flexibleWidth = 1;

            // Close Button (Absolute Top-Right)
            GameObject closeBtnObj = UIFactory.CreateButton("", header, Vector2.zero, new Vector2(20, 20), Color.clear, Color.white);
            ...
            cr.anchoredPosition = new Vector2(-15, -10); // Top-right offset
            
            // Try loading close icon from EditIcons first, then root
            Texture2D closeTex = Resources.Load<Texture2D>("EditIcons/p_close");
            if (closeTex == null) closeTex = Resources.Load<Texture2D>("p_close");
            ...
            if (closeTex != null)
            {
                ...
                closeBtnObj.GetComponent<Image>().color = Color.white; 
            }
            else
            {
                // Fallback text if icon missing
                UIFactory.CreateText("X", closeBtnObj, 14, Color.black, Vector2.zero, Vector2.zero);
            }
            ...
            cbr.offsetMin = new Vector2(24, 16); 
            cbr.offsetMax = new Vector2(-24, -50); 
            ...
            // Left: Preview (fill entire left column)
            GameObject leftCol = UIFactory.CreateObject("LeftCol", contentBody);
            LayoutElement leftLE = leftCol.AddComponent<LayoutElement>();
            leftLE.flexibleWidth = 0.45f;

            // Preview fills the entire left column
            GameObject previewMask = UIFactory.CreateObject("PreviewMask", leftCol);
            RectTransform pmRt = previewMask.GetComponent<RectTransform>();
            pmRt.anchorMin = Vector2.zero; pmRt.anchorMax = Vector2.one;
            pmRt.offsetMin = Vector2.zero; pmRt.offsetMax = Vector2.zero;

            Image maskImg = previewMask.AddComponent<Image>();
            maskImg.color = new Color(0.93f, 0.93f, 0.93f);
            if (inputSprite != null) {
                maskImg.sprite = inputSprite;
                maskImg.type = Image.Type.Sliced;
            }
            Outline previewBorder = previewMask.AddComponent<Outline>();
            previewBorder.effectColor = new Color(0.82f, 0.82f, 0.82f);
            previewBorder.effectDistance = new Vector2(1, -1);
            ...
            rawImg.color = new Color(0.93f, 0.93f, 0.93f);
            ...
            System.Func<string, GameObject, InputField> CreateInput = (label, p) => {
                ...
                rowLE.minHeight = 42; rowLE.preferredHeight = 42; rowLE.flexibleHeight = 0;
                ...
                GameObject labelObj = UIFactory.CreateText(label, row, 11, new Color(0.3f, 0.3f, 0.3f), Vector2.zero, Vector2.zero, TextAnchor.MiddleRight);
                LayoutElement lle = labelObj.AddComponent<LayoutElement>();
                lle.minWidth = 100; lle.preferredWidth = 100; lle.flexibleWidth = 0;
                ...
                // Text children must exist BEFORE InputField is added
                GameObject txtObj = UIFactory.CreateText("", inpObj, 10, Color.black, Vector2.zero, Vector2.zero);
                UIFactory.Stretch(txtObj.GetComponent<RectTransform>());
                txtObj.GetComponent<RectTransform>().offsetMin = new Vector2(6, 0);
                txtObj.GetComponent<RectTransform>().offsetMax = new Vector2(-6, 0);

                GameObject placeObj = UIFactory.CreateText("", inpObj, 10, new Color(0.6f, 0.6f, 0.6f), Vector2.zero, Vector2.zero);
                UIFactory.Stretch(placeObj.GetComponent<RectTransform>());
                placeObj.GetComponent<RectTransform>().offsetMin = new Vector2(6, 0);
                placeObj.GetComponent<RectTransform>().offsetMax = new Vector2(-6, 0);

                // Wire up InputField after children exist
            ...
            System.Func<string, GameObject, Dropdown> CreateDropdown = (label, p) => {
                ...
                UIFactory.AddDropdownArrow(ddObj, 10f);

                GameObject captionObj = UIFactory.CreateText("Option", ddObj, 10, new Color(0.6f, 0.6f, 0.6f), Vector2.zero, Vector2.zero);
                UIFactory.Stretch(captionObj.GetComponent<RectTransform>()); captionObj.GetComponent<RectTransform>().offsetMin = new Vector2(6, 0); captionObj.GetComponent<RectTransform>().offsetMax = new Vector2(-20, 0);
                dd.captionText = captionObj.GetComponent<Text>();
                
                // Template - positioned to align with the dropdown's left edge
                ...
                GameObject content = UIFactory.CreateObject("Content", vp);
                UIFactory.Stretch(content.GetComponent<RectTransform>());
                VerticalLayoutGroup cvlg = content.AddComponent<VerticalLayoutGroup>();
                cvlg.childControlHeight = false;
                cvlg.padding = new RectOffset(0,0,5,5); // Add padding
                cvlg.spacing = 2; // Add spacing
                content.AddComponent<ContentSizeFitter>().verticalFit = ContentSizeFitter.FitMode.PreferredSize;
                ...
                ir.sizeDelta = new Vector2(0, 24); // Standard item height
                
                Toggle toggle = item.AddComponent<Toggle>();
                
                // Item Background
                GameObject itemBg = UIFactory.CreateObject("ItemBg", item);
                UIFactory.Stretch(itemBg.GetComponent<RectTransform>());
                Image itemBgImg = itemBg.AddComponent<Image>();
                itemBgImg.color = Color.clear; // Transparent by default
                
                // Checkmark (Only show when selected)
                GameObject checkmark = UIFactory.CreateObject("Checkmark", item);
                ...
                cmRt.sizeDelta = new Vector2(14, 14); cmRt.anchoredPosition = new Vector2(12, 0);
                Image cmImg = checkmark.AddComponent<Image>();
                // Try to load a checkmark sprite
                Texture2D cmTex = Resources.Load<Texture2D>("EditIcons/p_check"); 
                if(cmTex == null) cmImg.color = new Color(0.2f, 0.8f, 0.4f); // Green square fallback
                else {
                    cmImg.sprite = Sprite.Create(cmTex, new Rect(0,0,cmTex.width,cmTex.height), Vector2.one*0.5f);
                ...
                GameObject itemLabel = UIFactory.CreateText("Option A", item, 11, Color.black, Vector2.zero, Vector2.zero);
                UIFactory.Stretch(itemLabel.GetComponent<RectTransform>());
                itemLabel.GetComponent<RectTransform>().offsetMin = new Vector2(30, 0); // Offset for checkmark
            ...
            GameObject btnRow = UIFactory.CreateObject("Buttons", rightCol);
            LayoutElement btnRowLE = btnRow.AddComponent<LayoutElement>();
            btnRowLE.minHeight = 36;
            btnRowLE.flexibleHeight = 0;
            btnRowLE.preferredHeight = 36;
            ...
            bhlg.padding = new RectOffset(0, 0, 5, 5); // Add padding to lift buttons off bottom edge slightly if needed

            Button cancelBtn = UIFactory.CreateButton("Cancel", btnRow, Vector2.zero, new Vector2(90, 30), Color.white, Color.black).GetComponent<Button>();
            ...
            Button pubBtn = UIFactory.CreateButton("Publish", btnRow, Vector2.zero, new Vector2(90, 30), new Color(0.1f, 0.1f, 0.1f), Color.white).GetComponent<Button>();
            ...
            logic.nameInput = nameInp;
            logic.categoryDropdown = catInp;
            logic.themeDropdown = themeInp;
            logic.styleDropdown = styleInp;
            logic.licenseDropdown = licenseInp;
            logic.tagsInput = tagsInp;
            logic.cancelButton = cancelBtn;
            logic.publishButton = pubBtn;

            logic.Setup(bridge, activeController.paper, () => Object.Destroy(dialogObj));
        }

        public static Texture2D CapturePaperFlatStatic(RectTransform paper)
        {
            return CapturePaperFlat(paper);
        }
